...
public class Files{
    ...
    boolean writeToFile(String filename,Table table){
        List<List<String>> temptable = table.getTable();
        try {
            File file = new File (filename);
            ...
            for(int i=0; i<temptable.size();i++){
                for (int j=0;j<temptable.get(i).size();j++) {
                    if(j==temptable.get(i).size()-1){
                        filewriter.write(temptable.get(i).get(j));
                    }
                    else {
                        filewriter.write(temptable.get(i).get(j));
                        filewriter.write(",");
    ...
    private void testFiles(){
        ...
        claim(testTable.selectRecord(3).get(0).equals("gh012"));
        claim(testTable.selectRecord(3).get(1).equals("Pete"));
