    ...
    void setmaxValueSizes(Table table) {
        ...
        for (int i = 0; i < table.getAttributesNum(); i++) {
            ...
            for (int j = 0; j < table.gettableSize(); j++) {
                if (temp[i]<table.selectRecord(table.getKeys().get(j)).get(i).length()) {
                    temp[i] = table.selectRecord(table.getKeys().get(j)).get(i).length();
                }
            }
    ...
    void printTable(Table table) {
        ...
        for (int j = 0; j < table.gettableSize(); j++) {
            for (int k = 0; k < table.getAttributesNum(); k++) {
                System.out.print("|");
                System.out.print(table.selectRecord(table.getKeys().get(j)).get(k));
                printSpace(maxes.get(k)-table.selectRecord(table.getKeys().get(j)).get(k).length());
            }
            System.out.print("|");
            System.out.println();
            printLine(countDash(table));
